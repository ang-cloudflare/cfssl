...
const (
	// Synthetic test-only root and intermediate pools generated by
	// testdata/build_certs.sh.
...
func TestBundleMarshalJSON(t *testing.T) {
	...
	if len(bundleCerts) != len(bundle.Chain) {
		t.Fatalf("JSON bundle contains %d certificates, want %d", len(bundleCerts), len(bundle.Chain))
	}
	for i, cert := range bundleCerts {
		if !bytes.Equal(cert.Raw, bundle.Chain[i].Raw) {
			t.Fatalf("JSON bundle certificate %d does not match Bundle.Chain", i)
		}
	}
	...
	if len(rootCerts) != 1 || bundle.Root == nil || !bytes.Equal(rootCerts[0].Raw, bundle.Root.Raw) {
		t.Fatal("JSON root does not match Bundle.Root")
	}
	...
	if len(certCerts) != 1 || !bytes.Equal(certCerts[0].Raw, bundle.Cert.Raw) {
		t.Fatal("JSON cert does not match Bundle.Cert")
	}
	if len(bundleCerts) == 0 || !bytes.Equal(certCerts[0].Raw, bundleCerts[0].Raw) {
		t.Fatal("JSON cert does not match the first JSON bundle certificate")
	}
...
// homogeneity when selecting a ubiquitous chain.
func TestSHA2HomogeneityAgainstUbiquity(t *testing.T) {
	leaf := readCert(leafECDSA256)
	sha1Intermediate := readCert(interL1SHA1)
	sha2Inter := readCert(sha2Intermediate)
	root := readCert(testCAFile)
	platformA := ubiquity.Platform{
		Name:            "A",
		Weight:          100,
		KeyStore:        make(ubiquity.CertSet),
	...
	platformA.KeyStore.Add(root)
	platformA.KeyStore.Add(sha2Inter)
	platformB.KeyStore.Add(root)
	originalPlatforms := ubiquity.Platforms
	ubiquity.Platforms = []ubiquity.Platform{platformA, platformB}
	t.Cleanup(func() { ubiquity.Platforms = originalPlatforms })

	moreUbiquitous := []*x509.Certificate{leaf, sha1Intermediate, root}
	moreHomogeneous := []*x509.Certificate{leaf, sha2Inter}
	chains := ubiquitousChains([][]*x509.Certificate{moreHomogeneous, moreUbiquitous})
	if len(chains) != 1 || !bytes.Equal(chains[0][len(chains[0])-1].Raw, root.Raw) {
		t.Fatal("SHA-2 homogeneity overrode platform ubiquity")
	}
}
